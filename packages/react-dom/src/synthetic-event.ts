...
function dispatchEvent(container: Container, eventType: string, e: Event) {
  ...
  // 1. 收集沿途的事件
  const { bubble, capture } = collectPath(targetElement as DOMElement, container, eventType);
  // 2，构造合成事件
...
function collectPath(targetElement: DOMElement, container: Container, eventType: string) {
  const paths: Paths = {
    capture: [],
    bubble: []
  };
